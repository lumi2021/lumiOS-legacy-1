...
const main = @import("../boot_main.zig");

pub fn load_kernel_image(
    file_system: *uefi.protocol.File,
    ...
    result = main.boot_services.freePool(header_buffer);
    if (result != uefi.Status.Success) { return result; }
    console.puts(" [done]\r\n");
    ...
    _ = main.boot_services.freePool(header_buffer);
    _ = main.boot_services.freePool(program_headers_buffer);

    return uefi.Status.Success;

}

fn read_file(file: *uefi.protocol.File, position: u64, size: usize, buffer: *[*]align(8) u8) uefi.Status {
    const result = file.setPosition(position);
    if (result != uefi.Status.Success) { return result; }

    return file.read(@constCast(&size), buffer.*);
}

fn read_and_allocate(file: *uefi.protocol.File, position: u64, size: usize, buffer: *[*]align(8) u8) uefi.Status {
    const result = main.boot_services.allocatePool(uefi.tables.MemoryType.LoaderData, size, buffer);
    if (result != uefi.Status.Success) { return result; }
...
fn load_segment(
    ...
    const num_pages = size_to_pages(memory_size);
    console.printf("pc: {}; ", .{num_pages});
    var seg_buffer: [*]align(4096) u8 = @ptrFromInt(virtual_address);
    var result = main.boot_services.allocatePages(
        uefi.tables.AllocateType.AllocateAddress,
        uefi.tables.MemoryType.LoaderData, num_pages, &seg_buffer
    );
    if (result != uefi.Status.Success) { return result; }

    console.printf("vaddr: ${X}; ", .{virtual_address});

    if(file_size > 0) {
        // load directly into correct position in memory
        console.printf("fs: ${X}; ", .{file_size});
    ...
    const zero_fill_start = virtual_address + file_size;
    const zero_fill_count = memory_size - file_size;

    if(zero_fill_count > 0) {
        console.printf("zfc: {}", .{zero_fill_count});
        main.boot_services.setMem(@ptrFromInt(zero_fill_start), zero_fill_count, 0);
    }
