...
pub fn open_file_descriptor(path: [:0]u8, flags: FileAccessFlags) isize {
    st.push(@src()); defer st.pop();
    ...
    } else return open_real_file_descriptor(current_task, path, flags);
}
...
fn open_real_file_descriptor(task: *Task, path: [:0]u8, flags: FileAccessFlags) isize {
    st.push(@src()); defer st.pop();
...
fn open_virtual_file_descriptor(task: *Task, path: [:0]u8, flags: FileAccessFlags) isize {
    st.push(@src()); defer st.pop();
